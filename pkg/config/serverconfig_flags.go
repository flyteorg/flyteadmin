...
func (cfg ServerConfig) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.Int(fmt.Sprintf("%v%v", prefix, "grpcConfig.grpcPort"), defaultServerConfig.GrpcConfig.Port, "On which grpc port to serve admin")
	cmdFlags.Bool(fmt.Sprintf("%v%v", prefix, "grpcConfig.grpcServerReflection"), defaultServerConfig.GrpcConfig.ServerReflection, "Enable GRPC Server Reflection")
	cmdFlags.Int(fmt.Sprintf("%v%v", prefix, "grpcConfig.maxGrpcMessageSizeBytes"), defaultServerConfig.GrpcConfig.MaxGrpcMessageSizeBytes, "The max size in bytes for incoming gRPC messages")
