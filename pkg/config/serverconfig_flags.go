...
func (cfg ServerConfig) GetPFlagSet(prefix string) *pflag.FlagSet {
	...
	cmdFlags.StringSlice(fmt.Sprintf("%v%v", prefix, "security.auth.userAuth.openId.aud"), []string{}, "")
	...
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "security.auth.appAuth.issuer.issuer"), defaultServerConfig.Security.Auth.AppAuth.Issuer.Issuer, "Defines the issuer to use when issuing and validating tokens. The default value is https://<requestUri.HostAndPort>/")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "security.auth.appAuth.accessTokenLifespan"), defaultServerConfig.Security.Auth.AppAuth.AccessTokenLifespan.String(), "Defines the lifespan of issued access tokens.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "security.auth.appAuth.tokenHashKeySecretName"), defaultServerConfig.Security.Auth.AppAuth.TokenHashKeySecretName, "OPTIONAL")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "security.auth.appAuth.tokenSigningRSAKeySecretName"), defaultServerConfig.Security.Auth.AppAuth.TokenSigningRSAKeySecretName, "OPTIONAL: Secret name to use to retrieve RSA Signing Key.")
	cmdFlags.String(fmt.Sprintf("%v%v", prefix, "security.auth.appAuth.oldTokenSigningRSAKeySecretName"), defaultServerConfig.Security.Auth.AppAuth.OldTokenSigningRSAKeySecretName, "OPTIONAL: Secret name to use to retrieve Old RSA Signing Key. This can be useful during key rotation to continue to accept older tokens.")
