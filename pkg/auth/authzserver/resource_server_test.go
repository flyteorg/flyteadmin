...
func newMockResourceServer(t testing.TB) ResourceServer {
	...
	*serverURL = s.URL
	...
	r, err := NewOAuth2ResourceServer(ctx, "https://localhost/", authConfig.ExternalAuthorizationServer{
		BaseURL: stdlibConfig.URL{URL: *mustParseURL(s.URL)},
	}, stdlibConfig.URL{})
...
func TestResourceServer_ValidateAccessToken(t *testing.T) {
	...
	_, err := r.ValidateAccessToken(context.Background(), sampleIDToken)
	assert.Error(t, err)
}
