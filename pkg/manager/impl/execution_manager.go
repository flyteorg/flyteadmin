...
func (m *ExecutionManager) launchSingleTaskExecution(
	...
	context.Context, *models.Execution, []models.ExecutionTags, error) {
	...
	return ctx, executionModel, nil, nil
}
...
func createTagsModel(project, domain, name string, tags []string) []models.ExecutionTags {
	tagsModel := make([]models.ExecutionTags, len(tags))
	for _, tag := range tags {
		tagsModel = append(tagsModel, models.ExecutionTags{
...
func (m *ExecutionManager) launchExecutionAndPrepareModel(
	...
	tagsModel := createTagsModel(request.Project, request.Domain, request.Name, request.Spec.Tags)

	return ctx, executionModel, tagsModel, nil
}
...
func (m *ExecutionManager) createExecutionModel(
	ctx context.Context, executionModel *models.Execution, tagsModel []models.ExecutionTags) (*core.WorkflowExecutionIdentifier, error) {
	...
	err := m.db.ExecutionRepo().Create(ctx, *executionModel, tagsModel)
	if err != nil {
...
func (m *ExecutionManager) CreateExecution(
	...
	*admin.ExecutionCreateResponse, error) {
	...
	var tagsModel []models.ExecutionTags
	var err error
	ctx, executionModel, tagsModel, err = m.launchExecutionAndPrepareModel(ctx, request, requestedAt)
	if err != nil {
		return nil, err
	}
	workflowExecutionIdentifier, err := m.createExecutionModel(ctx, executionModel, tagsModel)
	if err != nil {
		return nil, err
	}
...
func (m *ExecutionManager) RelaunchExecution(
	...
	*admin.ExecutionCreateResponse, error) {
	...
	var executionModel *models.Execution
	var tagsModel []models.ExecutionTags
	ctx, executionModel, tagsModel, err = m.launchExecutionAndPrepareModel(ctx, admin.ExecutionCreateRequest{
